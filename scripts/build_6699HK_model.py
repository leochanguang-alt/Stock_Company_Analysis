#!/usr/bin/env python3
"""
Financial Valuation Model Generator — 6699.HK 时代天使 (Angelalign Technology)
HK-listed Chinese clear-aligner company. Reports in HKD.
...
# ─── Config ───────────────────────────────────────────────────────────────────
TS_CODE    = '6699.HK'
HK_CODE5   = '06699'           # 5-digit HK code for akshare
COMPANY    = '时代天使'
CCY        = 'HKD'
UNIT_LABEL = 'HKD 亿元'
UNIT_DIV   = 1e8               # raw HKD → 亿 HKD

YEARS = list(range(2018, 2026))   # 8 annual years: 2018-2025

# CN 10yr sovereign bond yield (business operated in China; revenue in RMB)
# Using CN yields despite HKD reporting currency — underlying risk-free rate
CN_BOND = {
    2018: 0.0323, 2019: 0.0314, 2020: 0.0315, 2021: 0.0278,
...
for attempt in range(3):
    try:
        df_px = ak.stock_hk_hist(symbol=HK_CODE5, period="daily",
                                 start_date="20210101", end_date="20261231", adjust="")
...
if df_px is None or df_px.empty:
    ...
    try:
        ...
        t = yf.Ticker("6699.HK")
        hist = t.history(start="2021-01-01", end="2026-12-31")
        if not hist.empty:
            df_px = pd.DataFrame({
...
notes = [
    ...
    (8, ''),
    (9, 'Known key shareholders of 6699.HK 时代天使 (per 2024 Annual Report):'),
    (10, '  • EP Square Investment Limited — PE/founder holding vehicle'),
    (11, '  • Align Technology — 2019 strategic partner/investor'),
    (12, '  • Public float + other institutional holders'),
]
for r, text in notes:
...
doc = [
    ...
    (4,  '时代天使 (Angelalign Technology) — Chinese clear-aligner (invisible braces) manufacturer.', False),
    (5,  'IPO: June 2021 on HKEX main board. Ticker: 6699.HK.', False),
    (6,  'Reporting currency: HKD (converted from RMB by the issuer/data provider).', False),
    (7,  'Competes with Align Technology (Invisalign) and domestic clear-aligner peers.', False),
    (8,  '', False),
    ...
    (33, '• Only 8 years of history available (company IPO\'d in 2021; pre-IPO pro forma back to 2018).', False),
    (34, '• 2020-2022 dividends near zero (post-IPO company building reserves).', False),
    ...
    (40, '2018=3.23% 2019=3.14% 2020=3.15% 2021=2.78% 2022=2.84% 2023=2.56% 2024=1.68% 2025=1.83%', False),
    (41, '', False),
