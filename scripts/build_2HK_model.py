#!/usr/bin/env python3
"""
Financial Valuation Model Generator — 0002.HK 中电控股 (CLP Holdings Limited)
HK-listed regulated utility (HK + Australia + China + India + SE Asia electricity). Reports in HKD.
...
# ─── Config ───────────────────────────────────────────────────────────────────
TS_CODE    = '2.HK'
HK_CODE5   = '00002'           # 5-digit HK code for akshare
COMPANY    = '中电控股 (CLP Holdings)'
CCY        = 'HKD'
UNIT_LABEL = 'HKD 亿元'
...
# HK 10yr sovereign bond yield (HKD utility — use HKD/USD curve as cost of equity benchmark)
CN_BOND = {
    2015: 0.0162, 2016: 0.0144, 2017: 0.0205, 2018: 0.0228, 2019: 0.0173,
    2020: 0.0078, 2021: 0.0151, 2022: 0.0386, 2023: 0.0386, 2024: 0.0381, 2025: 0.0428,
}
...
for attempt in range(3):
    try:
        df_px = ak.stock_hk_hist(symbol=HK_CODE5, period="daily",
                                 start_date="20140101", end_date="20261231", adjust="")
...
if df_px is None or df_px.empty:
    ...
    try:
        ...
        t = yf.Ticker("0002.HK")
        hist = t.history(start="2014-01-01", end="2026-12-31")
        if not hist.empty:
            df_px = pd.DataFrame({
...
notes = [
    ...
    (8, ''),
    (9, 'Known key shareholders of 0002.HK 中电控股 (CLP Holdings) per latest disclosures:'),
    (10, '  • Kadoorie family — Sir Michael Kadoorie & associated trusts ≈ 30-35% (longstanding controlling shareholders)'),
    (11, '  • CLP Holdings has NO single dominant institutional holder beyond the Kadoorie family stake'),
    (12, '  • BlackRock, Vanguard, GIC and HK retail public float make up the remainder'),
]
for r, text in notes:
...
doc = [
    ...
    (4,  '中电控股 (CLP Holdings Limited) — regulated electricity utility based in Hong Kong since 1901.', False),
    (5,  'Listed on HKEX since 1973. Ticker: 0002.HK. Hang Seng Index constituent.', False),
    (6,  'Reporting currency: HKD. Operations: HK (CLP Power, Scheme of Control), Mainland China,', False),
    (7,  'Australia (EnergyAustralia), India, Taiwan, Thailand, Vietnam. Generation mix: gas/coal/nuclear/renewables.', False),
    (8,  'Controlling shareholder: Kadoorie family (~30-35%) via private trusts.', False),
    (8,  '', False),
    (9,  '2. Data Sources', True),
    (10, 'Primary: akshare', False),
    (11, '  stock_financial_hk_report_em(stock="00002", symbol=..., indicator="年度") — IS/BS/CF annual', False),
    ...
    (14, '  yfinance ticker "0002.HK" — daily prices (akshare HK price endpoint unstable)', False),
    (15, 'Note: tushare does NOT provide HK financial statements at non-premium tier;', False),
    (16, '      tushare hk_daily covers prices but financials are sourced via akshare here.', False),
    (15, '', False),
    ...
    (30, 'Bond yield: HK 10yr Exchange Fund Note (HKMA) used as risk-free; CLP is HKD-denominated regulated utility.', False),
    (31, '', False),
    (32, '5. Data Caveats', True),
    (33, '• 11 years of history (2015-2025). FY25 annual report typically released in late February 2026.', False),
    (34, '• HK Scheme of Control (SoC): CLP Power and HK Electric earn a regulated return on net fixed assets.', False),
    (35, '  Currently 8% permitted return on equity (2018-2033 SoC agreement).', False),
    (36, '• Australia (EnergyAustralia) is unregulated, exposed to merchant pricing — high earnings volatility.', False),
    (37, '• Quarterly TTM analysis NOT applicable — HK requires only semi-annual (H1) + annual disclosure.', False),
    (38, '• Top-10 shareholder quarterly grid NOT available via free HK APIs; see HKEX Disclosure of Interest.', False),
    (39, '• Capex is consistently large (HK$15-25B/year); FCF can be negative during heavy buildout cycles.', False),
    (40, '', False),
    (41, '6. Bond Yield Reference (HK 10yr Exchange Fund Note year-end)', True),
    (42, '2015=1.62% 2016=1.44% 2017=2.05% 2018=2.28% 2019=1.73% 2020=0.78% 2021=1.51% 2022=3.86% 2023=3.86% 2024=3.81% 2025=4.28%', False),
    (41, '', False),
