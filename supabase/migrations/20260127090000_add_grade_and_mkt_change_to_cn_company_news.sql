-- 给 cn_company_news 增加字段：grade, mkt_change_1_month

-- 0) 先移除依赖（否则后续 ALTER 可能被 view/rule/trigger 阻塞）
...
ALTER TABLE IF EXISTS stock_analysis.cn_company_news
  ...
  ADD COLUMN IF NOT EXISTS mkt_change_1_month DOUBLE PRECISION;

COMMENT ON COLUMN stock_analysis.cn_company_news.grade IS '新闻评级/打分（可为空）';
COMMENT ON COLUMN stock_analysis.cn_company_news.mkt_change_1_month IS '近1个月市场涨跌幅（可为空，单位建议：%）';
